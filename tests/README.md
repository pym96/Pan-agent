...
`test_live_tui.py` exercises WorkOrder #21's reusable product slice with zero real calls: the actual CLI confirmation boundary, multiple fresh-context Runs over one persistent workspace, accepted DeepSeek v3 Translation/Gateway composition with retained fake responses, native tool-result replay, semantic overflow recovery, active progress and offline replay, prompt return after cancellation/Provider failure, reasoning/credential display denial, traversal/absolute/symlink/unknown/malformed fail-closed behavior, atomic bounded text writes, and built-in non-executing syntax verification.
