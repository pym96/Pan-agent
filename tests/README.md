...
`test_live_tui.py` exercises WorkOrder #21's reusable product slice with zero real calls: identity-bound admission of non-authoritative textual content beside valid ToolCalls; hash-bound synthetic multi-call fixtures; one assistant turn with three Provider-ordered calls and three paired results; all-before-effects workspace preflight; terminal ambiguity rejection; the actual CLI confirmation boundary; multiple fresh-context Runs over one persistent workspace; semantic overflow recovery; active progress and replay; cancellation/failure return; display denial; path/unknown/malformed fail-closed behavior; atomic bounded writes; and non-executing syntax verification.
