...
test('C-TUI-A-R03 admitted history restores original draft caret and snapshots',()=>{
 ...
 f.ui.key(undefined,{name:'up'});assert.equal(f.ui.editor.text,'second');f.ui.key(undefined,{name:'up'});assert.equal(f.ui.editor.text,'first');f.ui.key(undefined,{name:'escape'});assert.equal(f.ui.editor.text,'unsent');assert.equal(f.ui.editor.caret,2);assert.deepEqual(f.ui.selected,[snap]);
});
