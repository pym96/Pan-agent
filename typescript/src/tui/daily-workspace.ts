...
import { DailyEditor, clip, wrap, graphemes, width } from './daily-editor.ts';
...
export class DailyWorkspace {
 ...
 private pending='';private pasting=false;private pasted='';private barrier=false;private readonly raw:boolean;
 ...
 start():void {this.input.setRawMode(true);this.output.write('\x1b[?1049h\x1b[?2004h\x1b[?25h');this.input.on('data',this.data);this.input.on('end',this.end);this.output.on('resize',this.resize);this.input.resume();this.draw();}
 dispose():void {clearTimeout(this.escapeTimer);this.picker?.abort.abort();this.input.off('data',this.data);this.input.off('end',this.end);this.output.off('resize',this.resize);this.keys.removeAllListeners();this.input.setRawMode(this.raw);this.input.pause();this.output.write('\x1b[0m\x1b[?2004l\x1b[?25h\x1b[?1049l');this.output.write(`Pan closed · Run ${terminalText(this.runId)} · Archives ${terminalText(this.options.archiveStore?.root??'not configured')}\n`);}
 ...
 private readonly data=(data:Buffer|string):void=> {
  ...
  while(this.pending){
   ...
   const at=this.pending.indexOf('\x1b[200~');if(at>=0){if(at)this.keys.write(this.pending.slice(0,at));this.pending=this.pending.slice(at+6);this.pasting=true;this.pasted='';this.closePicker(false);this.overlay=undefined;continue;}
   // Retain a split paste marker only, while readline handles ordinary split escape sequences.
   let keep=0;for(let n=1;n<=5;n++)if(this.pending.endsWith('\x1b[200~'.slice(0,n)))keep=n;
   const ready=this.pending.slice(0,this.pending.length-keep);this.pending=this.pending.slice(this.pending.length-keep);if(ready)this.keys.write(ready);if(keep){this.escapeTimer=setTimeout(()=>{const value=this.pending;this.pending='';this.keys.write(value);},40);return;}
  }
 };
 ...
 key(text:string|undefined,key:InputKey={}):void {
  ...
  if(this.picker){const p=this.picker;if(key.name==='escape'){this.closePicker(true);return;}if(key.ctrl&&key.name==='c'){this.closePicker(false);return;}if(enter){if(!key.meta)this.select();return;}if(p.capturing)return;
   if(key.name==='up'||key.name==='tab'&&key.shift)p.index=Math.max(0,p.index-1);else if(key.name==='down'||key.name==='tab')p.index=Math.min(Math.max(0,this.matches().length-1),p.index+1);else if(key.name==='left')p.editor.move(-1);else if(key.name==='right')p.editor.move(1);else if(key.name==='backspace'){p.editor.backspace();p.index=0;}else if(this.printable(text,key)){p.editor.insert(text!);p.index=0;}this.draw();return;}
  if(key.ctrl&&key.name==='c'){this.cancel();return;}
  if(key.name==='escape'){this.focus='composer';this.safePaste=false;this.draw();return;}
  if(key.name==='pageup'||key.name==='pagedown'){this.follow=false;this.top=Math.max(0,this.top+(key.name==='pageup'?-1:1)*Math.max(1,Math.floor((this.output.rows??24)/2)));this.draw();return;}
  if(key.ctrl&&key.name==='end'){this.follow=true;this.newOutput=false;this.draw();return;}
  ...
  if(key.name==='left')this.editor.move(-1);else if(key.name==='right')this.editor.move(1);else if(key.name==='up')this.editor.vertical(-1);else if(key.name==='down')this.editor.vertical(1);else if(key.name==='backspace')this.editor.backspace();else if(key.ctrl&&key.name==='u')this.editor.clear();else if(key.name==='home'||key.ctrl&&key.name==='a')this.editor.caret=0;else if(key.name==='end'||key.ctrl&&key.name==='e')this.editor.caret=this.editor.text.length;else if(this.printable(text,key))this.editor.insert(text!);this.draw();
 }
 ...
 private send():void {
  ...
  void this.options.session.runTask(prepared).then(result=>this.options.presentation!.settle(result),()=>{if(this.active)this.active.status='Partial response · local failure';this.notice='Local error · draft retained';}).finally(()=>{if(this.closing)this.finish();else{this.phase='idle';this.draw();}});
 }
 private async command(text:string):Promise<void> {
  ...
  try{
   ...
   else if(text===':help')this.overlay.lines.push('Enter sends an idle nonblank draft; busy Enter never queues.','Alt-Enter: newline. Ctrl-V: safe paste/edit mode; Esc exits it.','@ file picker; Enter selects only; Ctrl-P preview; Ctrl-R remove.','Tab: focus. PageUp/Down: conversation. Ctrl-End: follow tail.','Ctrl-C cancels once; :exit quits. :details / :runs / :replay ID are view only.');
 ...
 private observe(event:SessionObservation):void {
  if(event.type==='run.started'){this.runId=event.runId;const decoded=decodeAttachedTask(event.task);this.entries.push({role:'You',text:decoded?.prompt??event.task,status:decoded?`${decoded.attachments.length} attached snapshot(s)`:''});this.active={role:'Pan',text:'',status:'Waiting for model'};this.entries.push(this.active);}
 ...
 draw():void {
  ...
  if(!this.supported()){rows.push({text:'Pan · Resize terminal (min 40 x 12)'},{text:'Draft retained · Ctrl-C cancel'});cursor={row:Math.min(h-1,2),col:0};}
   ...
   const matches=this.matches(),p=this.picker;const pickerRows=p?Math.min(7,Math.max(3,h-5-composer-1)):0;const bodyRows=h-5-composer-pickerRows;
   ...
   else {const selectedTool=this.entries.filter(e=>e.role==='Tool')[this.toolCursor];const all=this.entries.flatMap(e=>[{text:`${this.focus==='transcript'&&e===selectedTool?'> ':''}${e.role} · ${e.status}`,kind:e.role},...e.text.split('\n').flatMap(s=>wrap(terminalText(s),w-2).map(text=>({text:'│ '+text,kind:e.role})))]);if(!all.length){const intro=this.phase==='confirm'?['Confirm provider and trusted-local workspace [y/N]', 'Provider: '+terminalText(this.options.provider),'Workspace: '+terminalText(this.options.workspace),'Host-user tools; cwd is not an OS sandbox.']:['Write a task. @ selects a file; Enter sends.'];all.push(...intro.flatMap(s=>wrap(s,w-2).map(text=>({text,kind:'Pan' as const}))));}if(this.revealTool&&selectedTool){this.follow=false;this.top=Math.max(0,all.findIndex(r=>r.text.startsWith('> Tool'))-Math.floor(bodyRows/2));}this.revealTool=false;if(this.follow)this.top=Math.max(0,all.length-bodyRows);else this.top=Math.min(this.top,Math.max(0,all.length-bodyRows));body=all.slice(this.top,this.top+bodyRows);}
   for(let i=0;i<bodyRows;i++)rows.push(body[i]??{text:''});
   if(p){const safe=terminalText(p.editor.text),before=terminalText(p.editor.text.slice(0,p.editor.caret));const col=graphemes(before).reduce((n,g)=>n+width(g),0);const start=Math.max(0,col-(w-5));let off=0,skipped=0;const visible=graphemes(safe).filter(g=>{const n=off;off+=width(g);if(n<start){skipped=off;return false;}return true;}).join('');rows.push({text:`File ${p.capturing?'Capturing':p.loading?'Loading':`${matches.length? p.index+1:0}/${matches.length}`} · Enter selects only`});cursor={row:rows.length,col:Math.min(w-1,2+col-skipped)};rows.push({text:'@ '+visible,kind:'focus'});const count=pickerRows-2;const begin=Math.max(0,p.index-count+1);for(let i=0;i<count;i++){const index=begin+i;rows.push({text:matches[index]===undefined?'':`${index===p.index?'>':' '} ${terminalText(matches[index]!)}`,kind:index===p.index?'focus':undefined});}}
