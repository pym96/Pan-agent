...
import { validate, shape, digest, fileDigest, insist, Refusal, resultShape, type Manifest, type Ledger, type Attempt, type Result, type Role } from './model.ts';
...
export async function run(config:unknown, root:string, resume=false, options:Options={}):Promise<object> {
  ...
  try {
    ...
    const proc=options.process??new OfflineProcess();const tracker=options.tracker??new OfflineTracker(join(root,'tracker'));
    ...
    while(true) {
      ...
      if(last && last.phase!=='recorded') a=last;
        ...
        if(last?.role==='regulator') {
          const result=tracker.lookup(last.key) as Result;
          ...
          if(result?.outcome!=='criterion_failed') {transition('needs_human',result?.outcome??'missing_verdict');return summary(l);}
      ...
      try {
        ...
        const result=read(join(dir,'result.json'));resultShape(result,m,a);
        insist(fileDigest(join(dir,result.evidence.name))===result.evidence.digest,'evidence_missing');
        const evidence=read(join(dir,result.evidence.name));
        insist(evidence.simulation==='SIMULATED' && evidence.candidate===result.candidate && evidence.role===a.role && evidence.attempt===a.number,'evidence_identity');
        if(a.role==='regulator')insist(result.candidate===a.candidate,'result_identity');
        ...
        if(!['handoff','accepted','criterion_failed'].includes(result.outcome)){transition('needs_human',result.outcome);return summary(l);}
        ...
        insist(digest(tracker.lookup(a.key))===a.resultDigest,'tracker_mismatch');
