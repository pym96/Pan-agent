...
> **Mixed verification state; not a new verified fact (2026-08-23):** Human accepted ADR-0009/0010/0011. Separate same-model Regulator reviews accepted the ordinary Runtime/Campaign/seed/configuration boundary through HF-20260820-022. The newer bash-only ReAct MVP, DeepSeek Adapter, Docker/SWE-bench environment receipts, frozen five-case matrix, and post-run reliability fixes are Working Agent candidates pending independent review. All 30 planned slots executed, with 29 task outcomes and one infrastructure/artifact failure; this is not a SWE-bench Lite score. Start at `AGENTS.md`; the bounded assignment lives in `docs/agents/current-assignment.md`.
