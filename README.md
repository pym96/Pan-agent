...
> **Mixed verification state; not a new verified fact (2026-08-24):** Human accepted ADR-0009/0010/0011/0012. Separate same-model Regulator reviews accepted the ordinary Runtime/Campaign/seed/configuration boundary through HF-20260820-022, and a sixth review reproduced the ReAct MVP ordinary candidate-Evidence boundary. All 30 ReAct slots executed, with 29 task outcomes and one infrastructure/artifact failure; this is not a SWE-bench Lite score. The 240-slot `protocol-reliability-v1` replay and its 75-call maximum-token sensitivity plus separately identified 25-call 16K extension have completed as Working Agent candidate Evidence; they still require a new independent review. Start at `AGENTS.md`; the bounded assignment lives in `docs/agents/current-assignment.md`.
