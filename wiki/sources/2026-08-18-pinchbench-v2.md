...
- The pinned `task_polymarket_briefing` has canonical manifest category `research` but frontmatter category `Research`. A catalog Adapter must either retain this source discrepancy or define an explicit normalization rule; silently claiming byte-for-byte metadata agreement would be false.
...
- The category discrepancy is capitalization-only at the pinned commit. It does not establish that later tags have the same metadata.
