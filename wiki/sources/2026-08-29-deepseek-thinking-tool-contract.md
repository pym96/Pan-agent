...
|---|---|---|---|
| 1 | Thinking mode supports tool calls since DeepSeek-V3.2; the model may interleave multiple thinking/tool-call rounds before a final answer | Tool Calls guide, 思考模式 section | Documented |
| 2 | Strict mode (`strict: true` per function, `/beta` base URL) works in both thinking and non-thinking modes | Tool Calls guide, strict 模式 section | Documented |
| 3 | With `tools` present, **all** historical `reasoning_content` must be fully replayed in every subsequent request — even turns without a tool call; the docs state failure to replay returns **400**. Without `tools`, `reasoning_content` is ignored if sent | Thinking Mode guide, 输入输出参数 / 工具调用 sections | Documented |
...
| 6 | Thinking mode does not support `temperature`/`top_p`/`presence_penalty`/`frequency_penalty` — setting them does not error but has no effect; the general reference further marks `frequency_penalty`/`presence_penalty` as deprecated | Thinking Mode guide, 输入输出参数; Chat Completions reference | Documented |
| 7 | Thinking defaults to **on** with `reasoning_effort` default `high`; request→actual effort mapping for `deepseek-v4-flash`/`deepseek-v4-pro`: `low→low, medium→high, high→high, xhigh→high, max→max` | Thinking Mode guide, 控制参数 section | Documented |
