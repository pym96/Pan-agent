...
## 2026-08-28 | Batch internalize: discovery mechanisms, benchmark landscape, entropy-question refinements

- `concepts/harness-mcp-skills.md` extended with two session-verified points: discovery differs per layer (tools are declared to the model via RPC/structured parameters; skills alone are files the model reads), and the standard asymmetry — tool side has MCP, model side has no equivalent standard, hence per-harness canonical layers.
