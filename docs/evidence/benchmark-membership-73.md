...
## Criteria1.1 additive repair — partial, pending SC-73-02

The Criteria1.0 failure, original archive/checksum and independent rejected Verdict remain unchanged. Master Criteria1.1: https://github.com/pym96/Pan-agent/issues/73#issuecomment-5760162094. New observations and missing decisions are detailed in [SC-73-02](../design/benchmark-membership-resolution.md#criteria11-continuation--sc-73-02-pending).

Artifacts added: source-role reader and negatives, append-only display-budget gate and negatives, frozen-base overlay using the original selector, 16-row ledger (14 judgments/2 unresolved), continuation lock and combined 35-file provenance. All actual input hashes are retained; original observations.json/source-provenance.json remain historical and unchanged. Decision semantics require independent review; tests only check declared structural invariants.

30 offline tests passed, including duplicate/missing/extra/unresolved decisions, shuffled order, original-reason and protected-score tampering, missing evidence, non-mutation, nested Solution sections, unknown YAML fields, invalid-role/encoding paths, UTF-8 limits and repeated-display charging. The reader initially guarded a source display at the global limit; a later AST inventory did not use that guard. The conservative bound exceeds the excerpt ceiling, so C-MEM-03 is **not claimed satisfied**. No answer disclosure was observed, but neither tests nor regexes establish universal semantic cleanliness.
...
## Criteria1.3 evidence — single bounded diagnostic, final ScopeChallenge

Criteria1.3 explicitly incorporates complete1.2. The new received/unclear Human receipt, controller exposure entry, frozen parser lock, eight-object whole-file provenance, structural report and source-located matrix are linked from the [tool README](../../scripts/benchmark-membership/README.md). Historical1.0/1.1/1.2 rejections, null receipt, original archives, source flags and source accounting incident remain unchanged.

All8 acquisitions completed with pinned Git blob and whole-file hash records before structural inference. Network response bytes now total88,575,411, including earlier requests and failures. The128MiB ceiling and5400-second preparation limit are prospective cumulative extensions; all prior3226.1660947799683 seconds remain charged. Display retains the existing ledger/carry and98304-byte ceiling, including structural outputs, repeated/static excerpts and the producer-budget refusal. Final totals and archive identity are in the SHA-bound Handoff.

The frozen parser's12 tests passed before real payload access. Four additional diagnostic/authority tests passed afterward: changed parser rejected before payload access, tree mismatch rejected before payload access, exact eight-path authority, and received/unclear never becoming an exclusion. The targeted total is16 tests. The one-pass CLI marker prevents accidentally re-running this controller diagnostic in the same staging directory. No second real-data parsing pass was used to repair unsupported results. Independent Regulator reconstruction remains separate work.

C-MEM-06 evidence is supplied for independent review, but C-MEM-01/02 completion is still absent: two source headers are unsupported and the input-to-request/scored-result relationship remains unknown. The real overlay still refuses before writing output. No fixed subset, selected-list/category delta, final feasibility matrix or Human acceptance stimulus is manufactured.

Per1.3's direction not to repeat the same incomplete check suite, unchanged Product/Reference/host suites were not rerun. Prior37 local tests and independent82+259 host results remain bound only to the rejected1.2 SHA, not transferred as a whole-criterion PASS. New parser/diagnostic tests, protected-byte/link/whitespace checks and cumulative records cover this additive diagnostic checkpoint. Full completed-artifact candidate-bound acceptance remains required if Master enables a conclusive continuation.

Raw inputs and GT are retained in a new `criteria13-continuation` archive under the existing external Evidence root, with controller-only access boundaries documented. Do not dump the archive into model context. The one-pass outcome is SC-73-03; do not repeat this audit without genuinely new Master evidence/authority. No facts, benchmark scores, campaign authorization or untouched-holdout claim follow.
