...
## Additive repair R91-01 — original Criteria1.0

The preceding section describes the initial candidate and its Builder checks, not acceptance. Independent Verdict rejected `53f40ea6910635e47b8cccb77e6d3ae694058f09`: C-SELECT-01 FAIL and C-SELECT-03 FAIL for the same image-value type defect; C-SELECT-02 PASS. Verdict `/Volumes/WD_BLACK/pan-agent/wo91-single-task-20260924/regulator-20260924/Verdict.md`, SHA256 `a462977a11b8778d09e0a54eb6cf39e428c881c7b6103f4a9e80aec995a24b7f`. [Master repair routing](https://github.com/pym96/Pan-agent/issues/91#issuecomment-5809104606). Original candidate, archive, red independent evidence and prior Handoff are retained; no history rewrite.

Reason: JavaScript RegExp.test coerces inputs to strings. A correctly signed image value `["sha256:<64 hex>"]`, including a nested singleton array, passed both regex checks. The signature was valid; the runtime shape was invalid. Original tests checked mapping/string/signature failures but missed this type class. Independent actual CLI probes exposed the gap, so the prior passing regressions did not establish the full invariant. This was not a signature bypass or actual Provider/Docker call.

Repair adds `typeof value === 'string'` before the digest regex in both selection.mjs and the duplicate cli.mjs check. Four actual CLI cases now cover singleton array, nested singleton array, null and object, alongside existing malformed string/mapping, tampering, full-five and single-task paths. Each negative signs its malformed payload with a synthetic trusted authority, then requires rejection with zero environment/credential/provider/verifier counters and no ledger/output directory. No schema redesign, Product/session/broker/policy/task/package or report-semantics change.

Builder red-before-fix:14 selected negative tests,12 pass/2 fail; singleton and nested arrays each failed “Missing expected rejection.” Raw red log and resulting synthetic execution artifacts remain. After repair, full pilot152 total/144 pass/8 existing skips/0 fail, including these cases and valid scalar/full-five/single-task/report/signature/cancellation/deadline/accounting paths. Container controls remain disabled. No real model, credential, Docker startup, official scoring or live activation. Seven old ledgers unchanged; all82 installed files still match package lock. Host feedback empty; host whole-package check again retains the existing root-extra BLOCK, subsequent stages not run.

Repair raw evidence is separate: `/Volumes/WD_BLACK/pan-agent/wo91-single-task-20260924/builder-repair/evidence.tar.gz`, SHA256 `fca9380d6fa01f0795362753a1d2f506012d3d1c3e28b89992c9359fed6e819a`; index `index.json`, SHA256 `8a4cb137e67c4420386e2996b85ac38d6f041c8d6dc4438966edd1ea897d9fef`. All639 entries rehashed from tar, including red/green logs, synthetic fixtures and identity checks. Working repair root `/private/tmp/wo91-single-task/repair/`. Original719-entry archive remains unchanged. New full candidate SHA is bound by the additive Handoff; independent probes remain outside candidate.

Learning correction: a regex is not a type guard, and a valid signature authenticates data without establishing its runtime schema. Builder's missing type predicate and narrow negative inputs caused the defect; Regulator's independent typed counterexample corrected that assumption. Explicit scalar checking closes this observed gate without claiming general schema completeness or any real task-performance benefit. Master signing instructions above remain applicable after independent acceptance; no live follow-up has started.
