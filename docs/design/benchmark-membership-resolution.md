...
## Criteria1.2 continuation — one Human input pending

Master's Criteria1.2 disposition permits only the additional `plot-line-006/tips.txt` task input, carries at least 72,488 display bytes into a prospective 98,304-byte ceiling, and retains the Criteria1.1 incident as an explicit audit limitation. It does not retroactively accept either rejected candidate.

The [new decision ledger](../../scripts/benchmark-membership/decisions-v12.json) contains **9 excluded, 6 eligible, 1 unresolved** Builder judgments. New tips evidence resolves plot-line-006: its complete normal task-input lines 1–7 specify month-to-season groups, country exclusions and country-name normalization. They provide no plot.yaml settings, generation instructions or source locator. The required configuration remains unprovided in the pinned distribution. This is a structural-provision judgment for independent review, not proof that nobody could invent a suitable plot configuration.

For plot-scatter-002, static scorer tracing rules out the earlier label-only semantic-conflict hypothesis. `evaluation.py:229–254` puts config under `options.config`; `metrics/image.py:170–225` only reads `options.keys`. The preserved task keys are x_label/y_label/labels, omitting type. `test_info:149–168` uses those keys; `compare_key:46–49` compares types only when that key is requested. Pixel/data comparisons still use official references, which were not inspected. Thus no enforced bar-versus-scatter incompatibility has been established; no semantic-conflict exclusion is claimed.

The explicit Solution section of `plot-scatter-002/guidance.txt` remains Human-only. [The pending receipt](../../scripts/benchmark-membership/human-metadata-v12.json) binds revision/path/SHA256 but contains no invented Human answer. Human must return yes/no/unclear on exact order-stage/city-sales provision and explicit upstream override, with metadata locators/line numbers only. The reply is a source input, not H-MEM-DECISIONS. Absence of a reply blocks only dependent resolution/selection. Independent gate and tips/scorer work has continued.

### Incident reconstruction and forward control

[incident-v12.json](../../scripts/benchmark-membership/incident-v12.json) records 19 prior guarded records totalling 31,528 reported bytes. Eighteen record payloads were reconstructed from retained projections/line selectors with matching byte counts; one static excerpt has only its old count/selector, not a preserved exact output. Ungated AST output and total historical display coverage remain unknown. Record hashes identify whole reconstructed non-answer records, not answer values. No old PASS or absence-of-leak conclusion is manufactured.

[source_view.py](../../scripts/benchmark-membership/source_view.py) is the sole new source-display entry point: pinned role/blob/size verification, source projection or explicitly named static functions, bounded excerpts, shared pre-display charge, durable exact-payload retention, then stdout. Errors use a generic counted view without exception data. Repeats and AST/source function excerpts use the same gate. Negative tests cover exhausted output, count reset, negative ledger charges, role bypass before source open, nested/mixed Solution sections, and retained stdout/receipt equality. Historical ad-hoc scripts remain evidence, not authorized new display routes.

Post-revision display receipts currently charge 18,255 bytes, giving 90,743 cumulative reserved/charged bytes, below 98,304; this is not a measurement of complete historical actual display. Every new displayed payload is retained with byte/line counts and whole-record hashes. The old 32,768 and 8,192 reservations are not discarded. Source responses accumulate to 114,097 bytes. Time/disk ledgers also continue without renewal.

Real overlay still refuses the single unresolved row before any output directory is created. No fixed 15+15 list, category delta or final selected-task feasibility matrix is claimed. Do not request H-MEM-DECISIONS until these exist; the eventual Human review must prominently include the old incident and prospective remediation.

## Criteria1.3 — one package audit completed; SC-73-03 unresolved

Current contract is the complete Criteria1.2 plus [the prospective1.3 amendment](https://github.com/pym96/Pan-agent/issues/73#issuecomment-5769854359). Human's [source-bound reply](https://github.com/pym96/Pan-agent/issues/73#issuecomment-5769835594) has been recorded as **received / provision unclear / override unclear**, without inventing locators. The original null receipt remains historical. Do not ask Human to repeat the Solution interpretation.

The authorized single controller-side diagnostic acquired and verified all eight enumerated objects against the pinned Git tree, recording whole-file SHA256. Parser whitelist and 12 synthetic tests were frozen before acquisition/GT access. Raw bytes remained outside solver/task inputs in the private controller staging directory; no task, scorer, model or image rendering was run. No input data row, GT label/title value, array value, pixel or solution content was displayed.

The [structural report](../../scripts/benchmark-membership/package-audit-v13.json) and [relationship matrix](../../scripts/benchmark-membership/package-matrix-v13.json) establish:

- Three input headers positively describe matches, teams, scores, leagues and odds. This observation is based on allowed column names, not filenames alone.
- `odds_series.csv.gz` and `odds_series_b.csv.gz` are **unsupported_or_malformed** under the frozen bounded compressed-CSV parser. This is a parser result, not proof that these files lack useful data. No second decoding attempt, row sampling or dynamic schema expansion followed.
- GT objects exist and yield supported safe JSON/NPY/PNG structural metadata. Explicit chart-kind metadata and numeric array/image shapes do not certify upstream correctness or task alignment.
- The official metric does not consume nested config.type as a chart-kind gate. Pixel equality may return before array checks; the array fallback requires shape and numeric comparison, and configured JSON fields affect grading. These value/pixel relationships cannot be established from permitted structural metadata alone. The upstream `allow_pickle=True` code was read statically, never called; the controller parser never unpickles.
- Official post-processing produces task plot.json/result.npy. Static bar capture collects rectangle width/height data, but no task-specific produced shape or values were generated. A complete producer excerpt exceeded remaining display capacity; a bounded source-located width/height/return excerpt was subsequently recorded through the same gate. Neither yields an invented bar/GT incompatibility proof.

**SC-73-03, final result of the authorized one-pass audit:** no source-backed order-stage/city-sales provision or derivation has been established across the entire package, because two schemas remain unsupported and Human metadata remains unclear. Separately, GT metadata and actual scorer consumers do not establish whether the required bar result can satisfy the official pixel/value contract. The task remains unresolved. The other15 judgments remain Builder judgments pending independent semantic review; no automatic exclusion is introduced for this controlled GT inspection.

Master must now decide a disclosed population/project boundary or a genuinely new source/evidence plan. Do not send Builder into another identical investigation, rerun this package pass, change the parser based on hidden contents, or silently exclude the unknown task. Formal 16-row completion and 15+15 selection remain blocked. H-MEM-DECISIONS remains premature.
