...
- [`pan-owned-canonical-protocol.md`](pan-owned-canonical-protocol.md): WorkOrder #31's pending-review Pan-owned Message/Tool/usage/identity/failure semantics plus the `ModelAdapter` and `AgentTool` seams now used by NativeKernel; Pi remains the default behind a named transitional compatibility module.
