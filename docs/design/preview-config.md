...
- [`src/config/keychain.ts`](../../typescript/src/config/keychain.ts): macOS `security` CLI wrapper over exact named service/account only — never enumerates. The production convention is `com.pym96.pan-agent` / `deepseek-api-key`; tests use the authorized disposable service `com.pym96.pan-agent.workorder-52-test` with a generated account. **The secret never appears in any child argv or environment**: the write runs `security -i` and delivers `add-generic-password … -w <secret>` through the stdin command channel (the child argv carries only `-i`); read/delete are argv operations without secret material. The `security` child receives a minimal explicit environment with the real user home (a redirected test HOME otherwise blocks keychain access). `security -i` keeps exit code 0 for failed commands, so failure is detected from the per-command `returned <code>` output marker; service/account are charset-validated before composing the command line, and the secret itself is validated as a single line of unquoted key characters (`A-Za-z0-9._~+/=-`) BEFORE any Keychain call — a newline/space/quote-bearing input is rejected with an explicit error, no partial write and no residue (this was the rejected candidate's C-CONFIG-04 failure: a line-split partial write). A `multiline-key` probe mode proves zero write-channel children and item absence.
...
- [config-keychain-driver.mjs](../../scripts/fixtures/preview/config-keychain-driver.mjs): disposable-item accept/decline/denied/fail-after-save/interrupt-after-save/multiline-key lifecycle with signal+finally cleanup and full child argv/env capture; the verifier independently re-checks the item's absence through the `security` CLI after every path.
...
- [check_workorder_52_scope.py](../../scripts/check_workorder_52_scope.py): exact changed-file inventory, byte-identical protection, 117 prior + 13 added test obligations, link checks, devtools isolation, no-Kimi-endpoint scan.
...
macOS + Node 22.19.0 only; TTY interaction; Kimi remains unavailable until a separately routed WorkOrder; trusted-local is not a sandbox; no real Provider credential, call, balance query or fee; no npm publication. Keychain writes use the interactive `security -i` stdin channel so no secret appears in any child argv or environment; verification instruments every child spawn/spawnSync and reports boolean canary containment over the captured argv/env.
