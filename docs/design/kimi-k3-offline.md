...
## R68-01 additive repair

Independent Regulator rejected `c27f8ca48917b69b1663242d5a098a7298ffe5e9` under C-K3-02: mutating a preceding message or replacing its object while the synthetic response was pending caused a second request with changed history and reasoning produced from the original request. After-admission mutation already refused, so that previous test did not cover the timing gap. The original Verdict/probe remains external and unchanged under `regulator-c27f8ca/` in the #68 evidence bundle.

The repair captures lineage before asynchronous work and rejects changed lineage before returning a canonical response (`protocol / kimi_continuation_unavailable`). New Builder regressions cover transport-pending and stream-pending content mutation, unequal/equal-content object replacement, appended history and unchanged controls, plus the real GeneralAgentSession boundary (one request, zero tool effects, one terminal). The installed verifier reuses these assertions against the compiled package; Regulator still supplies independent tests. No wire fixture, legacy assertion, protected module or Criterion was weakened. Request-time and response-time matching is not a general hostile-JavaScript isolation or transient-mutation detector.
