...
The [root README](../../README.md) now opens with what Pan lets a user do, a clear macOS early-preview boundary and the offline try path before any research/governance catalog, separating the two audiences explicitly: the **developer build** (`npm ci` + `npm pack`, one-time, locked dev dependencies) produces the exact local artifact; the **ordinary user try** [`scripts/try_preview.sh`](../../scripts/try_preview.sh) takes that artifact, installs it into a fresh consumer against an intentionally empty npm cache (`--omit=dev --offline --ignore-scripts --no-audit --no-fund`), confirms the installed executable, then [`scripts/demo_preview.mjs`](../../scripts/demo_preview.mjs) starts the actual installed Pan TUI through the public package surface. The user path builds nothing and downloads nothing.
