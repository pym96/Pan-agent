#!/usr/bin/env python3
"""#49 Criteria 1.2 installed Faux PTY, actual input, raw capture and guard meters."""
...
try:
 ...
 task('uncertain');pending();assert state()['approval']['request']['reason']=='resource_equivalence_uncertain';capture('uncertain-120');resize(40,12);capture('uncertain-40');resize(120,40);choose(1);assert (workspace/'new-uncertain/file.txt').read_text()=='APPROVED_UNKNOWN'
 task('uncertain-deny');pending();choose(0);assert not (workspace/'denied-uncertain').exists();capture('uncertain-denied-120')
 ...
 task('again');wait(lambda s:s.get('runs')==8 and s.get('phase')=='idle');assert not state()['approval'];capture('trusted-120')
 ...
 task('ordinary');wait(lambda s:s.get('runs')==12 and s.get('phase')=='idle');send('\t');send('\r');wait(lambda s:s.get('overlay',{}).get('title')=='Tool activity · view only');capture('activity-120');send('\x07');send('\x07')
 ...
 summary={'criteria':'1.2','result':'PASS','runs':12,'external_meters':'all zero','archive_before_after':before,'raw_sha256':hashlib.sha256(bytes(raw)).hexdigest(),'human_answers':'PENDING'}
