#!/usr/bin/env python3
"""Disposable current/historical snapshots and scope/graph negative controls; never edit candidate or prior retained paths."""
...
root=Path(__file__).resolve().parents[1];base='4cc479a16cd2767efda06c46f3d006f281e4c324';reports=[]
...
current=snapshot('HEAD','current');run('current',['node',str(root/'scripts/check_file_repair_scope.mjs'),str(current)])
for file,needle,name in [('typescript/src/tools/pan-trusted-local-tools.ts','protected file changed','current-tool'),('typescript/src/runtime/agent-kernel.ts','protected file changed','current-runtime')]:
 target=current/file;old=target.read_bytes();target.write_bytes(old+b'\n// unauthorized mutation control\n');run(name,['node',str(root/'scripts/check_file_repair_scope.mjs'),str(current)],1,needle);target.write_bytes(old)
...
historical=a.output/'historical-43';subprocess.run(['git','worktree','add','--detach',str(historical),base],cwd=root,check=True,stdout=subprocess.PIPE);shutil.copytree(root/'typescript/node_modules',historical/'typescript/node_modules',symlinks=True)
run('historical-43',['node',str(historical/'scripts/check_file_scope.mjs')])
for file,addition,needle,name in [('typescript/src/runtime/agent-kernel.ts','\n// unauthorized historical core mutation\n','protected file changed','historical-core'),('typescript/src/tui/presentation.ts','\nimport "../providers/deepseek/deepseek-profile.ts";\n','forbidden edge: tui -> providers','historical-graph')]:
 target=historical/file;old=target.read_bytes();target.write_bytes(old+addition.encode());run(name,['node',str(historical/'scripts/check_file_scope.mjs')],1,needle);target.write_bytes(old)
...
print('PASS current and exact #43 scopes with six negative controls; snapshots retained')
