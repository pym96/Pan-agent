...
for columns,rows in [(80,24),(40,12)]:
 for mode in ['busy','cancel','pre-confirm','idle-exit','safety']:
  ...
  child=subprocess.Popen([a.node,'--experimental-strip-types',str(root/'scripts/fixtures/streaming-baseline-pty-driver.mjs'),str(root),str(directory/'workspace'),str(directory/'memory'),mode,str(control_r),str(event_w)],stdin=slave,stdout=slave,stderr=slave,env=env,pass_fds=(control_r,event_w))
   ...
   if mode=='pre-confirm':send('\x03')
    ...
    if mode=='idle-exit':send('\x03')
     ...
     if mode=='safety':
      ...
      pump(lambda:raw.rfind('You > '.encode())>raw.rfind('Model error (model_error)'.encode()))
      ...
      send('\r');pump(lambda:has('Busy — draft retained'))
      # The blocked Tool proves no settlement/second admission exists yet.
      assert not any('settled' in e for e in events)
      os.write(control_w,b'tool\n');pump(lambda:any('settled' in e for e in events) and has('You > 草稿a中b'))
      screen=Screen(columns);screen.feed(raw.decode());assert 'You > 草稿a中b' in screen.text(),screen.text()
      ...
      pump(lambda:raw.rfind('You > '.encode())>raw.rfind('Cancelled (cancelled)'.encode()));send('after\r');pump(lambda:len([e for e in events if 'settled' in e])==2)
     send('\x03')
