...
assert.deepEqual(Object.keys(results[1].exports.product).filter(name => !(name in results[0].exports.product)), ['AttachmentError', 'DEFAULT_MAX_ATTACHMENT_BYTES', 'captureAttachment', 'decodeAttachedTask', 'discoverAttachmentPaths', 'prepareAttachedTask', 'validateAttachmentLimit']);
fs.writeFileSync(path.join(output, 'summary.json'), JSON.stringify({node: process.version, results}, null, 2) + '\n');
console.log('PASS all legacy public named exports plus the additive input preparation exports and the same typed client against both installed tarballs');
