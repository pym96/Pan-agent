...
assert.equal(adapter.state.exchangeCount,replayCounters);assert.match(rendered,/归档回放/);
assert.deepEqual(await archiveFiles(),snapshotBeforeReplay,'replay must not modify sealed bytes');
assert.match(rendered,/工具接纳数 3 · 工具启动事件数 3 · 工具返回数 3/);
assert.match(rendered,/工具接纳数 unavailable \(not recorded\) · 工具启动事件数 3 · 工具返回数 3/);
assert.match(rendered,/模型调用 unavailable \(not recorded\)/);
assert.match(rendered,/完整工具结果/);assert.match(rendered,/invalid ID/);assert.doesNotMatch(rendered,/RUN_SUMMARY|TOOL started|\x1b/);
