#!/usr/bin/env python3
"""#44 streaming-aware extension of #35 source→compiled tarball→production consumer proof, on the declared Node floor.
...
consumer=out/'consumer';consumer.mkdir();(consumer/'package.json').write_text(json.dumps({'name':'wo44-consumer','private':True,'type':'module','devDependencies':{}})+'\n')
...
for source,name in [('scripts/wo35-consumer-guard.mjs','guard.mjs'),('scripts/wo44-consumer-driver.mjs','driver.mjs'),('scripts/fixtures/packed-create-run-verify-v1.json','fixture.json'),('scripts/fixtures/streaming-replay-consumer.mjs','replay.mjs'),('scripts/fixtures/tui-archive-consumer.mjs','archives.mjs')]:shutil.copy2(ROOT/source,instrumentation/name)
