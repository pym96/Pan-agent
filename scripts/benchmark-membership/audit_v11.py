...
def audit(inputs):
    ...
    provenance=json.loads((ROOT/'source-provenance-v11.json').read_text())
    for r in provenance:
    ...
    return {'criteria':'1.1','source_files_verified':len(provenance),'decisions':{s:sum(d['decision']==s for d in decisions) for s in ('eligible','excluded','unresolved')},'selection_performed':False,'execution_authorized':False,'semantic_decisions_independently_accepted':False}
