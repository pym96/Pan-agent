...
assert.deepEqual(Object.keys(results[1].exports.product).filter(name => !(name in results[0].exports.product)), []);
fs.writeFileSync(path.join(output, 'summary.json'), JSON.stringify({node: process.version, results}, null, 2) + '\n');
console.log('PASS all legacy public named exports without export changes and the same typed client against both installed tarballs');
