...
output.on('data',(chunk)=>{
 ...
 if(chunk.includes('[y/N]> '))setImmediate(()=>input.write('y\n'));
  ...
  if(phase===0)setImmediate(()=>input.write('Create hello.js, run it and verify its exact source.\n'));
   ...
   actualRunId=/RUN_SUMMARY run=(\S+) terminal=completed model_calls=4 tool_calls=3 .*archive_sealed=true/.exec(rendered)?.[1];
   assert.ok(actualRunId,'actual Product result summary must report the completed sealed task');
   archiveFiles().then(files=>{snapshotBeforeReplay=files;replayCounters=adapter.state.exchangeCount;input.write(`:replay ${actualRunId}\n`);}).catch(error=>{throw error;});
...
assert.equal(adapter.state.exchangeCount,replayCounters);assert.match(rendered,/REPLAY run=/);
