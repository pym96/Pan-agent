...
console.log('OFFLINE TUI A · Compatibility: Ctrl-G Back; raw Esc reserves a sequence prefix. Ctrl-G during a pending prefix returns but drains its late tail; some raw typing may be consumed until resynchronized. Use paste for literal control data. · installed Product · Faux only. Type only y then Enter first. @ opens files; arrows choose; Tab attaches; Ctrl-P previews; separate Enter sends.');console.log('Wheel/trackpad (terminal SGR reports) scrolls; PageUp/Down fallback; Ctrl-End follows tail. Up/Down recalls prompts at draft edges. During the 8-second response, edit next draft; Ctrl-C cancels once. Ctrl-V safe paste/edit; Ctrl-G exits it. Ctrl-D exits even with pending input; :exit closes.');console.log('Local recording directory: '+root);
