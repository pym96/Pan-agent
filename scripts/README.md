...
## WorkOrder #51 product-first entry and offline first task

[`try_preview.sh`](try_preview.sh) is the documented offline first-task try: given an already-built local artifact it installs into a fresh consumer against an intentionally empty npm cache (`--omit=dev --offline --ignore-scripts --no-audit --no-fund`), then starts the installed offline preview — no build, no download; building the artifact is the separately documented developer step (`npm ci` + `npm pack`). [`demo_preview.mjs`](demo_preview.mjs) launches the actual installed Pan TUI with the scripted Faux first task under the network/credential guard; [`fixtures/preview/`](fixtures/preview/README.md) holds the deterministic check driver, fresh-process replay driver and interactive driver bound to [`fixtures/preview-first-task-v1.json`](fixtures/preview-first-task-v1.json). [`verify_preview_consumer.py`](verify_preview_consumer.py) proves the clean-consumer install, four-exchange task, fresh replay, canary isolation and zero meters; [`check_workorder_51_scope.py`](check_workorder_51_scope.py) audits the exact scope and 115 prior obligations. Design and Criterion mapping: [preview-first-task](../docs/design/preview-first-task.md). These are verification tools, not package runtime files; historical scripts remain unchanged.
