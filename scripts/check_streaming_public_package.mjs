...
assert.deepEqual(Object.keys(results[1].exports.product).filter(name => !(name in results[0].exports.product)), ['DeepSeekFetchTransport', 'PanDeepSeekModelAdapter', 'createPanDeepSeekAdapter']);
fs.writeFileSync(path.join(output, 'summary.json'), JSON.stringify({node: process.version, results}, null, 2) + '\n');
console.log('PASS all legacy public named exports plus the additive direct Adapter injection exports and the same typed client against both installed tarballs');
