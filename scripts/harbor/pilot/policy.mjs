...
export function budgets(value){check(value&&canonical(Object.keys(value).sort())===canonical(Object.keys(LIMITS).sort()),'budget_fields');for(const [k,max] of Object.entries(LIMITS))check(Number.isInteger(value[k])&&value[k]>0&&value[k]<=max,'budget_range');return Object.freeze({...value});}
...
export function authorize(activation,authority,expected,{clock=Date.now,signal}={}){
 ...
 const start=Date.parse(activation.notBefore),end=Date.parse(activation.expiresAt);check(Number.isFinite(start)&&Number.isFinite(end)&&start<end,'activation_window');
 const gate=Object.freeze({runId:activation.runId,binding:freeze(structuredClone(expected)),expiresAt:end,assert(otherSignal){check(!signal?.aborted&&!otherSignal?.aborted,'cancelled');check(clock()>=start&&clock()<end,'activation_expired');},clock});gate.assert();return gate;
}
...
export class Ledger{
 ...
 reserve(id,kind,signal){this.#gate.assert(signal);const t=this.#tasks.get(id),b=this.#gate.binding.budget;check(t,'attempt_not_started');if(kind==='dispatch'){check(t.dispatches<b.dispatchesPerTask&&this.#dispatches<b.dispatchesCampaign,'dispatch_budget');this.#write({event:'dispatch_reserved',task:id,taskNumber:t.dispatches+1,campaignNumber:this.#dispatches+1});t.dispatches++;this.#dispatches++;}else{check(kind==='tool'&&t.tools<b.toolsPerTask,'tool_budget');this.#write({event:'tool_reserved',task:id,number:t.tools+1});t.tools++;}}
