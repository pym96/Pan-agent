"""WO74 owned-command receipts and finite resource sampling; not a hard quota."""
import datetime, json, os, pathlib, signal, subprocess, time
GIB=2**30
RAW=pathlib.Path(os.environ.get('WO74_DOCKER_RAW', str(pathlib.Path.home()/'Library/Containers/com.docker.docker/Data/vms/0/data/Docker.raw')))
class Resources:
 ...
 def sample(self):
  if RAW.resolve().stat().st_dev != pathlib.Path('/').stat().st_dev:
   # macOS data/system APFS volumes can have distinct devices: compare /private/tmp.
   if RAW.resolve().stat().st_dev != pathlib.Path('/private/tmp').stat().st_dev:raise RuntimeError('Docker active disk is not internal')
  stat=os.statvfs(self.work);allocated=0
  if self.work.exists():
 ...
 def run(self,name,command,*,env,cwd=None,timeout=1200,estimate=0):
  ...
  for args in [['image','ls','--digests','--no-trunc','--format','{{json .}}'],['ps','-a','--format','{{.ID}} {{.Names}} {{.Image}} {{.Status}}'],['network','ls','--format','{{json .}}'],['volume','ls','--format','{{json .}}'],['system','df']]:
   r=subprocess.run(['docker',*args],env=env,capture_output=True,text=True,timeout=15);inventory.append({'args':args,'exit':r.returncode,'stdout':r.stdout,'stderr':r.stderr})
  ...
  with (self.evidence/(name+'.log')).open('wb') as log:
   ...
   try:
    while p.poll() is None:
     ...
     if elapsed>=timeout:raise RuntimeError('time_budget_exhausted')
...
if __name__ == '__main__':
 ...
 parser.add_argument('--work',required=True);parser.add_argument('--name',required=True);parser.add_argument('--timeout',type=int,default=1200);parser.add_argument('command',nargs=argparse.REMAINDER)
 a=parser.parse_args();work=pathlib.Path(a.work).resolve();work.mkdir(parents=True,exist_ok=True)
 env={'PATH':'/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin','HOME':str(work/'home'),'DOCKER_CONFIG':str(work/'docker-config'),'DOCKER_HOST':'unix://'+str(pathlib.Path.home()/'.docker/run/docker.sock'),'LANG':'en_US.UTF-8','PIP_CACHE_DIR':str(work/'pip-cache'),'UV_CACHE_DIR':str(work/'uv-cache'),'PYTHONDONTWRITEBYTECODE':'1'}
 pathlib.Path(env['HOME']).mkdir(exist_ok=True);pathlib.Path(env['DOCKER_CONFIG']).mkdir(exist_ok=True)
 Resources(work/'evidence',work).run(a.name,a.command,env=env,timeout=a.timeout)
