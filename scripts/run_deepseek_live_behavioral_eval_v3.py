#!/usr/bin/env python3
"""Preview v3 by default; live execution needs the exact v3 identity triple."""
...
class _LazyBalanceStore:
    """Delay filesystem creation until an acknowledged runner claims its root."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Preview the v3 120-slot campaign with zero external calls by default; "
            "live execution requires the exact printed v3 identity acknowledgement."
        )
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Enter the sole acknowledged production runner; never used for preview.",
    )
    parser.add_argument(
        "--acknowledgement",
        help="Exact execute-live v3 identity string required with --live.",
    )
    arguments = parser.parse_args(argv)
    lock = load_deepseek_live_eval_v3_lock()

    if not arguments.live:
        document = build_v3_zero_call_dry_run(lock=lock)
        body = json.dumps(
            document,
            ensure_ascii=False,
            sort_keys=True,
    ...
    if arguments.acknowledgement != required:
        parser.error("--live requires the exact v3 lock/runner/entry acknowledgement")

    # Credential access remains behind the exact v3 identity Gate. This Stage A
    # WorkOrder never enters this branch and grants no paid-execution authority.
