/** New process: replay an actually attachment snapshot with no task-file reads or effects. */
...
const compiled=product.endsWith('/pan-agent');const {runCli,runTui,FauxModelAdapter,RunArchiveStore,decodeAttachedTask}=await import(pathToFileURL(join(product,compiled?'dist/index.js':'typescript/src/index.ts')));
...
const before=await hashes();const store=await RunArchiveStore.open(memory),records=await store.readArchive(runId);const task=records.find(e=>e.type==='run.started').task;const snapshot=decodeAttachedTask(task);assert.ok(snapshot);const fixture=JSON.parse(await fsp.readFile(join(workspace,'..','fixture.json'),'utf8'));assert.equal(snapshot.attachments[0].text,fixture.content);
...
assert.equal(code,0);assert.equal(adapter.state.exchangeCount,0);assert.equal(effects,0);assert.equal(taskReads,0);assert.deepEqual(await hashes(),before);assert.match(text,/SNAPSHOT_ONLY/);assert.doesNotMatch(text,/UNSELECTED_CANARY|ENV_CANARY|MUTATED_AFTER_SELECTION/);assert.match(text,/Transient previews are not recorded/);assert.match(text,/Admitted tool calls unavailable \(not recorded\)/);assert.match(text,/Model calls unavailable \(not recorded\)/);
...
console.log('PASS fresh-process attachment snapshot replay, exact recorded snapshot, sealed hashes unchanged, zero task reads/exchanges/effects');
