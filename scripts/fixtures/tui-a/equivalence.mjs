...
for(const viewing of [false,true]){
 const directory=join(output,viewing?'view':'noop'),workspace=join(directory,'workspace');await mkdir(workspace,{recursive:true});
 ...
 function normalize(v,k=''){if(Array.isArray(v))return v.map(x=>normalize(x));if(v&&typeof v==='object')return Object.fromEntries(Object.entries(v).map(([key,x])=>[key,normalize(x,key)]));if(typeof v==='string')return ids.get(v)??v.split(workspace).join('WORKSPACE');if(k==='timestamp'&&typeof v==='number'&&v>1e12)return 'GENERATED_TIME';return v;}
...
await writeFile(join(output,'result.json'),JSON.stringify({result:'PASS',normalization:'Generated Run/Session IDs and generated timestamps only; known disposable workspace prefix normalized. No payload fields omitted.',variants},null,2)+'\n');console.log('PASS installed paired views/noop; 3 real local tools, unchanged sealed archive reads');
