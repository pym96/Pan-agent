...
let inputBytes=0;if(process.stdin.isTTY)process.stdin.on('data',b=>{inputBytes+=Buffer.byteLength(b);queueMicrotask(()=>notify({input:true}));});
...
const state=()=>({phase:ui?.phase,focus:ui?.focus,draft:ui?.editor.text,caret:ui?.editor.caret,query:ui?.picker?.editor.text??null,queryCaret:ui?.picker?.editor.caret,index:ui?.picker?.index,loading:ui?.picker?.loading??false,capturing:ui?.picker?.capturing??false,attachments:ui?.selected,overlay:ui?.overlay?.title??null,overlayOffset:ui?.overlay?.offset,entries:ui?.entries,top:ui?.top,anchor:ui?.anchor,history:ui?.history,historyIndex:ui?.historyIndex,stash:ui?.stash,follow:ui?.follow,newOutput:ui?.newOutput,safePaste:ui?.safePaste,inputState:ui?.inputState,inputBytes,admissions,exchanges,cancels,keyCount,outputBytes:bytes,results,reads});
