...
const readTargets=[],results=[],observations=[],contexts=[],keys=[],gates=new Map();let keyCount=0;
...
const oldFileOpen=fsp.open;fsp.open=async function(path,...args){const file=await oldFileOpen.call(this,path,...args);if(typeof path==='string'&&path.startsWith(workspace+'/')){opens++;readTargets.push({operation:'open',path:path.slice(workspace.length+1)});const oldRead=file.read.bind(file);file.read=async function(...args){const result=await oldRead(...args);reads++;bytesRead+=result.bytesRead;readTargets.push({operation:'read',path:path.slice(workspace.length+1),bytes:result.bytesRead});if(gateCapture){gateCapture=false;await barrier('capture');}return result;};}return file;};syncBuiltinESMExports();
