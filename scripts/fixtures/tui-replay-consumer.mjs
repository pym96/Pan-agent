...
assert.match(text,/ARCHIVE_ERROR corrupt-fixture/);assert.match(text,/LOCAL_ERROR/);assert.match(text,/工具启动事件数 1 · 工具返回数 0/);assert.match(text,/暂无可查看/);assert.match(text,/工具接纳数 unavailable \(not recorded\)/);assert.match(text,/模型调用 unavailable \(not recorded\)/);
for(const run of runs){const records=await store.readArchive(run.runId);const count=records.filter(r=>r.type==='tool.settled').length;assert.ok(text.includes(`工具已返回 ${count} 次`));}
