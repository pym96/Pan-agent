/** #60 installed scrollbar PTY driver: real DailyWorkspace, long streamed Faux transcript, per-event timing records. */
import fs from 'node:fs';
import fsp from 'node:fs/promises';
import {join} from 'node:path';
import {pathToFileURL} from 'node:url';
const [product,workspace,memory]=process.argv.slice(2);
if(!product||!workspace||!memory)throw new Error('usage: scrollbar-pty-driver.mjs INSTALLED_PACKAGE WORKSPACE MEMORY');
...
const wheelTimings=[],mouseTimings=[],dimensions=[];
...
const oldMouse=DailyWorkspace.prototype.mouse;
DailyWorkspace.prototype.mouse=function(action,x,y){const t0=performance.now();const r=oldMouse.apply(this,arguments);mouseTimings.push({action,x,y,ms:performance.now()-t0,top:this.top,follow:this.follow,dragging:this.dragging,newOutput:this.newOutput,builds:this.layoutStats.builds,visits:this.layoutStats.sourceRowVisits,rows:this.contentRows.length,bodyHeight:this.bodyHeight});return r;};
...
const adapter={providerId:'pan-faux (offline/simulated)',modelId:'pan-faux-v1',reasoningLevel:'off',async exchange(request){
 ...
 const text=index===0?long:'Offline scrollbar demo: replay is view only. Use :runs, :replay RUN_ID or :exit.';
...
console.log('OFFLINE / SIMULATED SCROLLBAR DEMO · installed Product · long transcript · wheel/keys plus primary press/drag/release on the final-column track · resize freely.');
console.log('Confirm y, then enter any short task (e.g. demo) to receive an 800-line transcript, then scroll or drag the scrollbar.');
console.log('Local recording directory: '+recordsRoot);
const report=()=>({keyCount,admissions,exchanges,networkAttempts,outputBytes,dimensions,wheelTimings,mouseTimings,layoutStats:ui?.layoutStats??null,top:ui?.top,follow:ui?.follow,newOutput:ui?.newOutput,dragging:ui?.dragging,entries:ui?.entries.length,terminal:process.env.TERM});
process.on('SIGUSR2',async()=>{
 await fsp.writeFile(join(recordsRoot,'scrollbar-report.json'),JSON.stringify(report(),null,2)+'\n');
 process.exit(0);
});
const code=await runCli(['--kernel','native','--workspace',workspace,'--memory-root',memory],{createNativeAdapter:()=>adapter});
await fsp.writeFile(join(recordsRoot,'scrollbar-report.json'),JSON.stringify({code,...report()},null,2)+'\n');
process.exit(code);
