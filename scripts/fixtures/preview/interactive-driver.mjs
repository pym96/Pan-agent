...
if(!product||!workspace||!memory||!fixturePath)throw new Error('usage: interactive-driver.mjs INSTALLED_PACKAGE WORKSPACE MEMORY FIXTURE [SETTINGS_HOME]');
...
let exchanges=0,canned=0,resolvedProfile=null;
const adapter={providerId:'pan-faux (offline/simulated)',modelId:'pan-faux-v1',reasoningLevel:'off',async exchange(request){ const index=exchanges++;
...
if(settingsHome)console.log('Settings home (disposable): '+settingsHome);
const deps={createNativeAdapter:profile=>{resolvedProfile=profile;if(profile)console.log(`Persisted selection restored: ${profile.modelId} / ${profile.thinkingLevel} (simulated by pan-faux)`);return adapter;}};
if(settingsHome)deps.home=settingsHome;
const code=await runCli(['--kernel','native','--workspace',workspace,'--memory-root',memory],deps);
await fsp.writeFile(join(recordsRoot,'report.json'),JSON.stringify({code,fixture:fixture.id,exchanges,canned,networkAttempts,outputBytes,terminal:process.env.TERM,resolvedProfile,settingsHome:settingsHome??null},null,2)+'\n');
