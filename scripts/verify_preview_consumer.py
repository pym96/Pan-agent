#!/usr/bin/env python3
"""#51 product-first entry: source→compiled tarball→clean consumer→offline first task→fresh replay proof.
...
# The independent expected result is derived here, outside the model-visible workspace.
...
canaries={'values':['CANARY-PREVIEW-FAKE-DEEPSEEK-KEY-7f3d2a19c84e','CANARY-PREVIEW-FAKE-OPENAI-KEY-b51e09d27a64'],'note':'synthetic provider-key canaries; never real credentials'}
consumer=out/'consumer';consumer.mkdir();(consumer/'package.json').write_text(json.dumps({'name':'wo51-consumer','private':True,'type':'module','devDependencies':{}})+'\n')
...
for source,name in [('scripts/wo35-consumer-guard.mjs','guard.mjs'),('scripts/fixtures/preview/check-driver.mjs','driver.mjs'),('scripts/fixtures/preview/replay-driver.mjs','replay.mjs'),('scripts/fixtures/preview-first-task-v1.json','fixture.json')]:shutil.copy2(ROOT/source,instrumentation/name)
...
# C-ENTRY-02: deterministic first task with synthetic provider-key canaries only in the parent environment.
canary_env={'DEEPSEEK_API_KEY':canaries['values'][0],'OPENAI_API_KEY':canaries['values'][1]}
run([node,instrumentation/'driver.mjs'],consumer,'installed-tracer',{**guarded(consumer,'tracer'),**canary_env})
report=json.loads((consumer/'tracer-report.json').read_text());assert report['model_exchanges']==4 and report['tool_names']==['write','bash','read'] and report['canary_leaks']==0
run_id=report['run_id']
verify_installed(installed)
# C-ENTRY-03: a fresh installed process replays the sealed run with zero exchanges/effects/mutations.
run([node,instrumentation/'replay.mjs',installed,consumer/'workspace',consumer/'memory',run_id,consumer/'replay-report.json'],consumer,'installed-replay',{**guarded(consumer,'replay'),**canary_env})
...
# C-ENTRY-05: canaries reach no consumer surface, including package and evidence files.
# The definition file itself is the only permitted occurrence.
...
controls={
 ...
 'caught-network':"try { await fetch('https://example.invalid/wo51-negative-control'); } catch {}",
}
...
phases={r['phase'] for r in nominal};assert {'install','npm-ls','bin-help','bin-omitted','bin-pi','bin-unknown','bin-native-decline','tracer','replay','missing-runbook'}<=phases
...
save('summary.json',{'source_sha':sha,'development_dirty_source':dirty,'tarball_sha256':filehash(archive),'node':'v22.19.0','case':'preview-first-task/v1','run_id':run_id,'consumer':str(consumer),'installed_packages':physical,'nominal_meter_totals':{key:sum(r[key] for r in nominal) for key in keys},'negative_controls':{r['phase']:{key:r[key] for key in keys} for r in negative_reports},'tracer_report_sha256':filehash(consumer/'tracer-report.json'),'replay_report_sha256':filehash(consumer/'replay-report.json'),'canary_leaks':0,'runtime_files_unchanged':True,'package_or_toolchain_fetches':'none in consumer; build dependencies/toolchain supplied beforehand'})
print('PASS Node 22.19.0 preview consumer checks; retained evidence:',out)
