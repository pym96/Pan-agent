...
child=subprocess.Popen([a.node,str(root/'scripts/demo_tui.mjs'),'--package',a.package],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,env=env)
raw=bytearray()
...
try:
 ...
 start=len(raw);send('cancel\n');until('等待模型返回',start);send('\x03');until('你 › ',start);command(':details');command(':runs')
 import re
 ...
 text=raw.decode();assert 'PAN_PACK_OK' in text and 'line-200' in text and '已取消' in text and '模型错误' in text and '归档回放' in text
 assert 'Faux exchanges=9 · network attempts=0' in text
 (a.output/'transcript.txt').write_text(text);(a.output/'summary.json').write_text(json.dumps({'product':a.package,'builder_trial_only':True,'faux_exchanges':9,'network_attempts':0,'scenarios':['frozen','long','error','cancel'],'human_criterion':'NOT_EVALUABLE'},indent=2)+'\n')
 print('PASS actual Product offline interactive demo: frozen/long/error/cancel/details/replay (Builder observation only)')
finally:
