...
const repository=fileURLToPath(new URL('../',import.meta.url)),root=path.resolve(process.argv[2]??repository),base='28b524deafaef3494a5c1865c5ad68c5635f1f74';
...
const files=git('ls-tree','-r','--name-only',base).trim().split('\n');const map=JSON.parse(read('docs/design/workorder-43-obligations.json'));assert.equal(map.base,base);assert.equal(map.criteria_version,'1.0');
...
const permittedOld=f=>f.startsWith('typescript/src/tui/')||['typescript/src/cli.ts','typescript/src/index.ts','docs/agents/current-assignment.md','typescript/test/cutover-contract.test.ts'].includes(f)||f.endsWith('README.md');
const permittedNew=f=>/^typescript\/(src\/(input|tui)|test)\//.test(f)||/^scripts\//.test(f)||/^docs\/design\/(native-file-input\.md|workorder-43-obligations\.json)$/.test(f);
...
assert.equal(obligations.length,92);assert.deepEqual(map.prior_obligations,obligations);
...
console.log(JSON.stringify({base,changed,protectedHashes,priorObligations:obligations.length},null,2));console.log('PASS #43 scope, protected bytes, current graph, 92 prior obligations and document links');
